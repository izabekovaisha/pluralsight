...
public class DataManager {
    ...
    public void insertShipper(String companyName, String phoneNumber) {
        try (Connection connection = dataSource.getConnection();
             PreparedStatement statement = connection.prepareStatement("INSERT INTO shippers (CompanyName, Phone) VALUES (?, ?)",
            ...
            System.err.println("Error inserting shipper: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void updateShipperPhoneNumber(int shipperID, String phoneNumber) {
        try (Connection connection = dataSource.getConnection();
             PreparedStatement statement = connection.prepareStatement("UPDATE shippers SET Phone = ? WHERE ShipperID = ?")) {
            statement.setString(1, phoneNumber);
            statement.setInt(2, shipperID);
            statement.executeUpdate();
        } catch (SQLException e) {
            System.err.println("Error updating shipper phone number: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void deleteShipper(int shipperId) {
        try (Connection connection = dataSource.getConnection();
             PreparedStatement statement = connection.prepareStatement("DELETE FROM shippers WHERE ShipperID = ?")) {
